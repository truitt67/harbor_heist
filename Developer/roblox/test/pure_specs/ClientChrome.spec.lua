...
describe("EPIC 44 client chrome source contracts", function()
	...
	describe("kqbq.11 HUD income-line split labels", function()
		...
		it("readyLabel uses claim green Color3", function()
			-- harborheist-kqbq: migrated from hardcoded Color3.fromRGB(50, 160, 80)
			-- to Theme.color.status.claimReady token (same value, single source).
			expect(clientSource:find("Theme.color.status.claimReady", 1, true)).to.be.a("number")
		end)
	...
	describe("kqbq.18.2 uniform disabled treatment", function()
		...
		it("claim button is never disabled for DataStore health (etj2.2.6)", function()
			-- The kqbq.18.2-era health gate ("SAVING UNAVAILABLE" + disable) was
			-- removed: the server honors claim in degraded mode and the persistent
			-- banner is the truthful signal. Claim may only be enabled/disabled by
			-- income state, never by state.dataStoreHealthy.
			expect(clientSource:find("setButtonEnabled(claimButton, false, Theme.color.status.disabled)", 1, true) == nil).to.equal(true)
		end)
	...
	describe("kqbq.19.2 minigame tap acknowledgment (neutral flash + haptic)", function()
		...
		it("tap ack flash renders above the source minigame frame (derived ZIndex), still below reveal card", function()
			expect(clientSource:find("flash.ZIndex = (srcFrame and srcFrame.ZIndex or OVERLAY_Z_BASE) + 1", 1, true)).to.be.a("number")
		end)
	end)
	...
	describe("kqbq.22.2 currency formatting consistency (formatCash everywhere)", function()
		it("formatCash defines the million-plus abbreviation tier", function()
			expect(clientSource:find("if abs > 999999 then", 1, true)).to.be.a("number")
		end)
		it("millions abbreviate to M via 1e6 scaling", function()
			expect(clientSource:find('scaled, suffix = n / 1e6, "M"', 1, true)).to.be.a("number")
		end)
		it("billions tier defined (unreachable under MAX_COINS = 999,999,999 but policy-complete)", function()
			expect(clientSource:find('scaled, suffix = n / 1e9, "B"', 1, true)).to.be.a("number")
		end)
		it("K tier intentionally unused (sub-million always renders in full)", function()
			expect(clientSource:find("/ 1e3", 1, true) == nil).to.equal(true)
		end)
		it("fractional sub-1000 values keep one decimal (small incomes never read $0)", function()
			expect(clientSource:find("if n % 1 ~= 0 and abs < 1000 then", 1, true)).to.be.a("number")
		end)
		...
		it("old raw reveal income pattern is gone", function()
			expect(clientSource:find("formatCash(value or 0), incomePerMin)", 1, true) == nil).to.equal(true)
		end)
		it("sell/store prompt STORE button income routes through formatCash", function()
			expect(clientSource:find('sellStoreStoreBtn.Text = string.format("STORE $%s/min", formatCash(fish.IncomePerMinute or 0))', 1, true)).to.be.a("number")
		end)
		it("no raw %.1f/min income formatting remains on swept surfaces", function()
			expect(clientSource:find("$%.1f/min stored", 1, true) == nil).to.equal(true)
			expect(clientSource:find('"$%d  •  $%.1f/min"', 1, true) == nil).to.equal(true)
		end)
		it("collection card value + income route through formatCash", function()
			expect(clientSource:find("formatCash(data.baseSellValue or 0), formatCash(data.incomePerMinute or 0)", 1, true)).to.be.a("number")
		end)
		it("sell-all breakdown routes through formatCash (no raw $%d each)", function()
			expect(clientSource:find("$%d each", 1, true) == nil).to.equal(true)
			expect(clientSource:find("formatCash(rarity.value)", 1, true)).to.be.a("number")
		end)
		it("HUD cash label routes through formatCash", function()
			expect(clientSource:find('cashLabel.Text = "$" .. formatCash(target)', 1, true)).to.be.a("number")
		end)
		it("shop price routes through formatCash", function()
			expect(clientSource:find('"$" .. formatCash(entry.item.cost', 1, true)).to.be.a("number")
		end)
		it("sell confirm routes through formatCash", function()
			expect(clientSource:find('string.format("SELL ALL $%s? TAP", formatCash(payout))', 1, true)).to.be.a("number")
		end)
	...
	describe("etj2.2.6 DataStore-degraded persistent banner", function()
		it("banner frame + truthful contract copy exist", function()
			expect(clientSource:find('dataStoreBanner.Name = "DataStoreBanner"', 1, true)).to.be.a("number")
			expect(clientSource:find("Saving interrupted — retrying", 1, true)).to.be.a("number")
			expect(clientSource:find("Everything you earn counts and saves automatically when connection recovers. Stay in game to keep recent progress.", 1, true)).to.be.a("number")
			expect(clientSource:find("Saving restored — you're all caught up.", 1, true)).to.be.a("number")
		end)
		it("render() drives banner visibility off state.dataStoreHealthy", function()
			...
			-- load-order: render would bind a nil global otherwise).
			local defPos = clientSource:find('dataStoreBanner.Name = "DataStoreBanner"', 1, true)
			expect(defPos < renderPos).to.equal(true)
			expect(renderPos < showPos).to.equal(true)
		end)
		it("old false copy and one-shot-toast machinery are gone", function()
			expect(clientSource:find("Saving unavailable -- try again", 1, true) == nil).to.equal(true)
			expect(clientSource:find("SAVING UNAVAILABLE", 1, true) == nil).to.equal(true)
			expect(clientSource:find("dataStoreWarningShown", 1, true) == nil).to.equal(true)
		end)
		it("CLAIM is not gated on DataStore health", function()
			expect(clientSource:find("storeHealthy", 1, true) == nil).to.equal(true)
			-- The pulse loop must not carry a health carve-out either.
