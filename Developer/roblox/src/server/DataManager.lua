...
-- E2E test seam (TASK 19.6): swap the DataStore handles so the E2E runner
-- can inject an in-memory mock. Studio blocks ALL DataStore access for
-- unpublished local builds (GetDataStore throws "You must publish this
-- place to the web"), which leaves dataStore/dataStoreV1 nil here and turns
-- save() into a silent no-op — the mock lets the suite exercise the real
-- save/load/sanitize/migration logic in that environment. Pass nil to
-- restore the real handles captured at module load.
local realDataStore, realDataStoreV1 = dataStore, dataStoreV1
function DataManager._setStores(v2, v1)
	if v2 == nil then
		dataStore, dataStoreV1 = realDataStore, realDataStoreV1
	else
		dataStore, dataStoreV1 = v2, v1
	end
end
...
local function sanitize(data)
	...
	-- E2E 19.6: hoisted OUT of the `data.Aquarium` block above — the legacy
	-- v1 `capacityLevel` conversion (top of sanitize) sets UpgradeLevel on
	-- payloads that have NO Aquarium sub-table, so the recompute never ran
	-- and migrated players silently kept the tier-1 default Capacity (20)
	-- despite a higher UpgradeLevel. StateSync.getCapacity reads Capacity as
	-- authoritative, so the invariant Capacity == tiers[UpgradeLevel] must
	-- hold on EVERY path, not just v2 payloads.
