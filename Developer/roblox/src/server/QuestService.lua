...
local function progressQuests(session, predicate, incrFn)
	...
	local changed, grantedDaily = processList(session, session.dailyQuests, "Daily quest", predicate, incrFn)
	local weeklyChanged, grantedWeekly = processList(session, session.weeklyQuests, "Weekly quest", predicate, incrFn)
	changed = changed or weeklyChanged
	if changed then
		if stateSync then
			stateSync.push(session)
		end
		-- harborheist-sde2: checkpoint when a quest reward was granted. Every
		-- other economy path (store/sell/claim/purchase/raid) persists its
		-- mutation immediately; quest rewards were the lone gap — without this,
		-- coins + completion flags rode only the 60s autosave or the leave
		-- save, and a crash/leave in that window silently replayed the grant.
		-- Spawned so the caller returns immediately; coalesced by
		-- DataManager.save's isSaving/saveDirty guard, so a multi-quest
		-- completion still costs at most one trailing write.
		if (grantedDaily or grantedWeekly) and dataManager and session.player then
			task.spawn(function()
