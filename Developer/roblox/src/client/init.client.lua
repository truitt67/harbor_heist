...
local function applyScrollbarAutoHide(scrollingFrame)
	...
	local fadeTween = nil
	local idleTask = nil
	-- harborheist-review-aug2026-6yp6.2: capture + return the connection so
	-- callers CAN disconnect — was fire-and-forget, violating the disconnect-
	-- all-connections best practice (frames are session-lifetime today, so
	-- this is defensive trackability, not an active-leak fix).
	local conn = scrollingFrame:GetPropertyChangedSignal("CanvasPosition"):Connect(function()
		if fadeTween then
			fadeTween:Cancel()
			fadeTween = nil
		end
		if idleTask then
			task.cancel(idleTask)
			idleTask = nil
		end
		if scrollingFrame.ScrollBarImageTransparency > 0 then
			TweenService:Create(scrollingFrame, EASE_FAST, { ScrollBarImageTransparency = 0 }):Play()
		end
		idleTask = task.delay(1.2, function()
			idleTask = nil
			fadeTween = TweenService:Create(scrollingFrame, EASE_FAST, { ScrollBarImageTransparency = 1 })
			fadeTween:Play()
		end)
	end)
	return conn
...
-- harborheist-3mo7.1.3: scroll affordance indicators. Auto-hidden scrollbars
-- (kqbq.9) leave users no idle cue that a list scrolls — the bar only
-- appears mid-scroll, after discovery. Add thin gradient fades at the
-- top/bottom edges that are visible whenever content overflows in that
-- direction, and fade out when the edge is reached.
--
-- Design notes:
--   * Indicators are SIBLINGS of the ScrollingFrame, not children — a child
--     inside a ScrollingFrame scrolls with the canvas. Positions derive
--     from the list's UDim at construction (deterministic; AbsoluteSize is
--     not valid until the next layout pass).
--   * Appearance rides the FRAME's BackgroundTransparency (a plain float —
--     always tweenable). The UIGradient keeps a STATIC transparency shape
--     (opaque at the anchored edge, fading into the list); the frame
--     transparency multiplies it in. This matches the TextFadeEdge pattern
--     (~:2456) — tweening a gradient's NumberSequence property has no
--     precedent in this codebase and would throw inside a scroll handler
--     if the tween target were rejected.
--   * EASE_FAST (0.12s) is the project's fast token — the bead's "0.15s"
--     predates the Theme.motion rule (no bare TweenInfo literals).
--   * Frames default to Active=false, so the indicators never intercept the
--     touch scroll gesture.
--   * Returns the connection table (same trackability contract as
--     applyScrollbarAutoHide — lists are session-lifetime today).
local SCROLL_AFFORDANCE_HEIGHT = 24

local function applyScrollAffordance(scrollingFrame)
	local parent = scrollingFrame.Parent

	-- Static gradient shape: opaque at the anchored edge, transparent into
	-- the list. Frame BackgroundTransparency (tweened) fades it in/out.
	local VISIBLE = NumberSequence.new({
		NumberSequenceKeypoint.new(0, 0),
		NumberSequenceKeypoint.new(1, 1),
	})

	local function makeIndicator(name, side)
		local indicator = Instance.new("Frame")
		indicator.Name = name
		indicator.BorderSizePixel = 0
		indicator.BackgroundColor3 = Theme.color.surface.primary
		indicator.BackgroundTransparency = 1 -- hidden until refresh() shows it
		indicator.Size = UDim2.new(
			scrollingFrame.Size.X.Scale, scrollingFrame.Size.X.Offset,
			0, SCROLL_AFFORDANCE_HEIGHT)
		if side == "top" then
			indicator.Position = UDim2.new(
				scrollingFrame.Position.X.Scale, scrollingFrame.Position.X.Offset,
				scrollingFrame.Position.Y.Scale, scrollingFrame.Position.Y.Offset)
		else
			indicator.Position = UDim2.new(
				scrollingFrame.Position.X.Scale, scrollingFrame.Position.X.Offset,
				scrollingFrame.Position.Y.Scale + scrollingFrame.Size.Y.Scale,
				scrollingFrame.Position.Y.Offset + scrollingFrame.Size.Y.Offset - SCROLL_AFFORDANCE_HEIGHT)
		end
		indicator.ZIndex = scrollingFrame.ZIndex + 1
		indicator.Parent = parent

		local gradient = Instance.new("UIGradient")
		gradient.Color = ColorSequence.new(Theme.color.surface.primary, Theme.color.surface.primary)
		-- Rotation 90 = opaque at top fading down; 270 = opaque at bottom
		-- fading up.
		gradient.Rotation = side == "top" and 90 or 270
		gradient.Transparency = VISIBLE
		gradient.Parent = indicator
		return indicator
	end

	local topIndicator = makeIndicator("ScrollAffordanceTop", "top")
	local bottomIndicator = makeIndicator("ScrollAffordanceBottom", "bottom")

	local topShown, bottomShown = false, false
	local topTween, bottomTween = nil, nil

	local function setState(indicator, shown, prevTween)
		if prevTween then
			prevTween:Cancel()
		end
		local tween = TweenService:Create(indicator, EASE_FAST,
			{ BackgroundTransparency = shown and 0 or 1 })
		tween:Play()
		return tween
	end

	local function refresh()
		-- AbsoluteCanvasSize settles post-layout with AutomaticCanvasSize;
		-- both size signals and CanvasPosition feed this.
		...
		local canScroll = canvasH > viewH + 1
		local showTop = canScroll and scrollY > 1
		local showBottom = canScroll and scrollY < canvasH - viewH - 1
		if showTop ~= topShown then
			topShown = showTop
			topTween = setState(topIndicator, showTop, topTween)
		end
		if showBottom ~= bottomShown then
			bottomShown = showBottom
			bottomTween = setState(bottomIndicator, showBottom, bottomTween)
		end
	end

	local conns = {}
	table.insert(conns, scrollingFrame:GetPropertyChangedSignal("CanvasPosition"):Connect(refresh))
	table.insert(conns, scrollingFrame:GetPropertyChangedSignal("AbsoluteCanvasSize"):Connect(refresh))
	table.insert(conns, scrollingFrame:GetPropertyChangedSignal("AbsoluteSize"):Connect(refresh))
	refresh()
	return conns
end
...
local function makeOverlayFrame(name, titleText, subtitleText)
	...
	-- harborheist-kqbq.6: brightened glow — 18px wide, 0.50 transparency
	-- (was 15px / 0.75) so the timing marker is readable at a glance.
	markerGlow.Size = UDim2.new(0, 18, 1, 10)
	...
	markerGlow.BackgroundTransparency = 0.50
...
local function makeButton(parent, props)
	...
	local function setupPressFeedback()
		...
		-- harborheist-kqbq.2: dedicated press easing (Quad Out, 0.10s)
		pressTween = EASE_PRESS
		...
		button.MouseButton1Down:Connect(function()
			...
			-- harborheist-kqbq.2: depth 0.94 for a tactile click read
			activeTweens.scale = TweenService:Create(button, pressTween, { Scale = 0.94 })
		...
		if not IS_MOBILE then
			button.MouseEnter:Connect(function()
				...
				activeTweens.scale = TweenService:Create(button, EASE_HOVER, { Scale = 1.02 })
				activeTweens.scale:Play()
				...
				activeTweens.color = TweenService:Create(button, EASE_HOVER, { BackgroundColor3 = hoverColor })
				activeTweens.color:Play()
				
				-- harborheist-g9z2: Show hover glow overlay
				if hoverGlow then
					activeTweens.glow = TweenService:Create(hoverGlow, EASE_HOVER, { BackgroundTransparency = 0.9 })
				...
				for i, shadow in ipairs(hoverShadows) do
					...
					activeTweens["shadow_" .. i] = TweenService:Create(shadow, EASE_HOVER, { BackgroundTransparency = targetAlpha })
			...
			button.MouseLeave:Connect(function()
				...
				activeTweens.scale = TweenService:Create(button, EASE_HOVER, { Scale = 1 })
				activeTweens.scale:Play()
				
				-- harborheist-g9z2: Restore original color
				activeTweens.color = TweenService:Create(button, EASE_HOVER, { BackgroundColor3 = originalColor })
				activeTweens.color:Play()
				
				-- harborheist-g9z2: Hide hover glow
				if hoverGlow then
					activeTweens.glow = TweenService:Create(hoverGlow, EASE_HOVER, { BackgroundTransparency = 1 })
					activeTweens.glow:Play()
				end
				...
				-- harborheist-8qv8: track these in activeTweens so cancelTweens() covers them
				for i, shadow in ipairs(hoverShadows) do
					activeTweens["shadow_" .. i] = TweenService:Create(shadow, EASE_HOVER, { BackgroundTransparency = 1 })
					activeTweens["shadow_" .. i]:Play()
				end
...
local setButtonEnabled = function(button, enabled, disabledBg)
	if enabled then
		button.Active = true
		local savedBg = button:GetAttribute("kqbq18_2_savedBg")
		if savedBg then
			button.BackgroundColor3 = savedBg
			button.TextColor3 = button:GetAttribute("kqbq18_2_savedFg")
			button:SetAttribute("kqbq18_2_savedBg", nil)
			button:SetAttribute("kqbq18_2_savedFg", nil)
		end
	else
		if not button:GetAttribute("kqbq18_2_savedBg") then
			button:SetAttribute("kqbq18_2_savedBg", button.BackgroundColor3)
			button:SetAttribute("kqbq18_2_savedFg", button.TextColor3)
		end
		button.Active = false
...
-- harborheist-3mo7.3.34: compact empty state for SUBSECTIONS inside a list
-- (e.g. the MILESTONES section of the collection panel). The full
-- renderEmptyState hero card (200-220px, gradient icon tile) is designed
-- for panel-level empties — dropping it under a subsection header would
-- dwarf the rows around it. renderEmptyRow matches makeMilestoneRow's
-- visual language (surface.secondary, corners.roomy, stroke 0.9) at a
-- compact height with an accent icon tile + title + subtitle, so a
-- subsection reads as "designed and intentional" instead of "bare label".
local function renderEmptyRow(parent, cfg)
	local rowH = IS_MOBILE and 64 or 58
	local row = Instance.new("Frame")
	row.Name = "EmptyRow"
	...
	tile.Position = UDim2.new(0, 12, 0.5, -16)
	tile.BackgroundColor3 = accent
	tile.BackgroundTransparency = 0.20
	tile.ZIndex = 27
	tile.Parent = row
	corner(tile, Theme.corners.snug)
	...
	makeLabel(row, {
		...
		Position = UDim2.new(0, 56, 0, IS_MOBILE and 10 or 8),
		Text = cfg.title,
		Font = Theme.type.fonts.bold,
		TextSize = Theme.type.sizes.sm,
		TextColor3 = Theme.color.text.primary,
	...
	makeLabel(row, {
		...
		Position = UDim2.new(0, 56, 0, IS_MOBILE and 32 or 30),
		Text = cfg.subtitle,
		Font = Theme.type.fonts.body,
...
-- harborheist-kqbq.11: split the "ready" segment into its own label so the
-- 5Hz pulse can tween TextTransparency directly (zero string allocation)
-- instead of rebuilding the entire RichText line every 0.2s.
incomeLabel.Visible = false -- superseded by incomeRateLabel + readyLabel
local incomeRateLabel = makeLabel(hud, {
...
local readyLabel = makeLabel(hud, {
...
local function updateIncomeLine()
	local ready = state and state.unclaimedIncome or 0
	incomeRateLabel.Text = formatIncomeRate(state and state.incomePerSec or 0)
	if ready > 0 then
		readyLabel.Text = string.format("  •  $%s ready", formatCash(ready))
		-- Position readyLabel after the rate text and constrain its width
		-- to the remaining HUD card space (prevents overflow on mobile).
		local rateW = math.floor(incomeRateLabel.TextBounds.X)
		readyLabel.Position = UDim2.new(0, 14 + rateW, 1, IS_MOBILE and -22 or -26)
		readyLabel.Size = UDim2.new(1, -(14 + rateW), 0, 16)
...
-- harborheist-kqbq.11: pulse readyLabel.TextTransparency directly (zero
-- string allocation on the 5Hz path). 2.4s Sine triangle, same curve as
-- the old RichText alpha — just driven via a property instead of a rebuild.
task.spawn(function()
	while true do
		if state and (state.unclaimedIncome or 0) > 0 then
			local phase = (os.clock() % 2.4) / 2.4
			local alpha = 0.05 + 0.5 * (1 - math.abs(phase * 2 - 1))
			readyLabel.TextTransparency = alpha
			task.wait(0.2)
		else
			task.wait(0.5)
		end
...
local function showToastDirect(message, color, category, actions)
	...
	-- harborheist-review-aug2026-6yp6.1: store the connection so dismiss()
	-- can disconnect it — previously orphaned per toast, leaking the closure
	-- and its captured wrap/toast refs for the rest of the session.
	...
	accentBar.Size = UDim2.new(0, 6, 1, -14)
	...
	-- harborheist-kqbq.5: soft category glow behind the accent bar — 10px
	-- wide (2px out each side), 0.85 transparency, sits below the bar (Z 56).
	local accentGlow = Instance.new("Frame")
	accentGlow.Size = UDim2.new(0, 10, 1, -10)
	accentGlow.Position = UDim2.new(0, 6, 0, 5)
	accentGlow.BackgroundColor3 = color
	accentGlow.BackgroundTransparency = 1
	accentGlow.ZIndex = 56
	accentGlow.Parent = toast
	corner(accentGlow, Theme.corners.thin)
	...
	TweenService:Create(accentGlow, EASE_OUT, { BackgroundTransparency = 0.85 }):Play()
	...
	local function dismiss()
		...
		dismissed = true
		-- harborheist-review-aug2026-6yp6.1: kill the AbsoluteSize tracker
		-- before the fade — after wrap:Destroy() it would hold refs to dead
		-- instances. Guarded so a double-dismiss can't re-disconnect.
		if sizeConn then
			sizeConn:Disconnect()
			sizeConn = nil
		end
		if toast.Parent then
			...
			TweenService:Create(accentGlow, fade, { BackgroundTransparency = 1 }):Play()
...
-- ============================================================
-- harborheist-3mo7.3.15: prompt exit-animation machinery. Both inline
-- prompts (onboarding + sell/store comparison) ENTER with a UIScale
-- 0.92→1 EASE_POP but hard-hid via raw Visible=false — the last
-- asymmetric modals (panels/banners/context menu/toasts all animate
-- out). The exit mirrors the entrance: UIScale 1→0.92 (EASE_IN —
-- exits accelerate, R4 #7/#8 precedent) + fade every visual element,
-- Visible=false deferred to the tween's Completed.
--
-- Rest-alpha capture/restore (context-menu precedent, 3mo7.3.40): an
-- exit that fades children to transparency 1 must restore every rest
-- value on re-show, or the second open renders as a ghost.
-- ============================================================
local PROMPT_EXIT_SCALE = 0.92 -- mirrors the entrance seed

-- Capture the "at rest" alpha of every visual element in a subtree.
-- GuiObject covers Frame/TextLabel/TextButton (BackgroundTransparency);
-- text-bearing classes add TextTransparency; UIStroke uses Transparency.
local function captureRestAlphas(root)
	local rest = {}
	local function capture(inst)
		local entry = {}
		if inst:IsA("GuiObject") then
			entry.background = inst.BackgroundTransparency
		end
...
-- Tween every captured rest alpha toward fully transparent. Returns the
-- tween list so the caller can Cancel on re-show — a snap-restore alone
-- can't stop an in-flight exit tween pulling back to 1.
local function fadeAlphasToTransparent(rest, easing)
	local tweens = {}
	for inst, entry in pairs(rest) do
		if inst.Parent then
			local props = {}
			...
			if next(props) ~= nil then
				local tween = TweenService:Create(inst, easing, props)
				table.insert(tweens, tween)
				tween:Play()
			end
		end
...
-- Snap every captured alpha back to its rest value (re-show after an
-- exit or a canceled mid-exit — instant, the entrance does the motion).
local function restoreAlphas(rest)
	for inst, entry in pairs(rest) do
		if inst.Parent then
			if entry.background ~= nil then
				inst.BackgroundTransparency = entry.background
			end
			if entry.text ~= nil then
				inst.TextTransparency = entry.text
			end
			if entry.transparency ~= nil then
				inst.Transparency = entry.transparency
			end
		end
	end
end

-- Exit-animation state for one prompt. Fields: token (generation guard
-- for the show/hide/show race), hiding, rest (captured alphas), tweens
-- (in-flight exit tweens). captureRest() must run AFTER the prompt is
-- fully constructed (buttons, stroke, applyElevation shadows).
local function makePromptExit(prompt)
	local exitState = {
		token = 0,
		hiding = false,
		rest = nil,
	...
	function exitState.captureRest()
		exitState.rest = captureRestAlphas(prompt)
	end
	-- Cancel a wind-down and snap back to rest (re-show path).
	function exitState.cancel()
		exitState.token += 1
		for _, tween in ipairs(exitState.tweens) do
			tween:Cancel()
		end
		table.clear(exitState.tweens)
		exitState.hiding = false
		if exitState.rest then
			restoreAlphas(exitState.rest)
		end
		-- The exit may have pulled the scale below 1 — snap it back so a
		-- canceled wind-down doesn't strand the prompt mid-shrink.
		local scale = prompt:FindFirstChildOfClass("UIScale")
		if scale then
			scale.Scale = 1
		end
	end
	-- Scale 1→0.92 + fade everything; defer Visible=false to Completed.
	-- Returns true if the exit started (false = already hiding/hidden,
	-- so callers can run their own fallback for side effects).
	function exitState.exit(onComplete)
		if exitState.hiding or not prompt.Visible then
			return false
		end
		exitState.hiding = true
		exitState.token += 1
		local token = exitState.token
		local scale = prompt:FindFirstChildOfClass("UIScale") or Instance.new("UIScale")
		scale.Parent = prompt
		exitState.tweens = fadeAlphasToTransparent(exitState.rest, EASE_IN)
		local tween = TweenService:Create(scale, EASE_IN, { Scale = PROMPT_EXIT_SCALE })
		tween.Completed:Connect(function()
			exitState.hiding = false
			-- Token mismatch = a re-show superseded this exit; Parent nil
			-- = the prompt was destroyed mid-wind-down.
			if exitState.token == token and prompt.Parent ~= nil then
		...
		table.insert(exitState.tweens, tween)
		tween:Play()
		return true
	end
	return exitState
...
-- Forward declarations — the exit states are built AFTER both prompts
-- are fully constructed (rest capture needs every child, including
-- applyElevation's shadow layers). The show/dismiss/hide functions
-- above/below reference these upvalues; assignment happens at module
-- load before any runtime call (same pattern as hidePanels).
local onboardingExit
local sellStoreExit
...
onboardingDismiss.Activated:Connect(function()
	...
	-- harborheist-3mo7.3.15: animated exit — Visible=false is deferred to
	-- the tween's Completed (was a raw hard-hide).
	onboardingExit.exit()
end)
...
local function showOnboardingPrompt(stage, text, color)
	...
	-- harborheist-3mo7.3.15: a re-show during the exit wind-down (Visible
	-- still true) must cancel the exit — otherwise the deferred
	-- Visible=false would strand the prompt invisible and the entrance
	-- gate below would never fire again. cancel() snaps the alphas back
	-- to rest; the prompt stays visible as-is (no entrance replay — it
	-- never left).
	if onboardingExit.hiding then
		onboardingExit.cancel()
		return
	end
...
local function dismissOnboardingPrompt(stage)
	...
	currentPromptStage = nil
	-- harborheist-3mo7.3.15: animated exit (deferred Visible=false).
	onboardingExit.exit()
end
...
local sellStoreSellBtn = makeButton(sellStorePrompt, {
	...
	CornerRadius = Theme.corners.roomy, -- harborheist-3mo7.3.16: was raw 10
	ZIndex = 31,
})
...
-- harborheist-3mo7.3.15: build the exit states NOW — both prompts are
-- fully constructed (labels, buttons, stroke, applyElevation shadows),
-- so captureRest() records every visual element that must fade on exit
-- and snap back on re-show. Forward-declared upvalues (see above the
-- onboarding section) are assigned here, at module load, before any
-- runtime call touches them.
onboardingExit = makePromptExit(onboardingPrompt)
onboardingExit.captureRest()
sellStoreExit = makePromptExit(sellStorePrompt)
sellStoreExit.captureRest()
...
local function hideSellStorePrompt()
	-- harborheist-3mo7.3.15: animated exit — scale 1→0.92 + fade, with
	-- Visible=false deferred to the tween's Completed (was a raw
	-- hard-hide). exit() no-ops when the prompt isn't visible (e.g. the
	-- render-path call on a hidden prompt).
	sellStoreExit.exit()
...
local function showSellStorePrompt(fish)
	...
	-- harborheist-3mo7.3.15: a re-show during the exit wind-down (a new
	-- catch landing while the prompt scales out) must cancel the exit —
	-- the deferred Visible=false would otherwise strand it invisible.
	-- cancel() snaps alphas + scale back to rest; the entrance below
	-- re-seeds the pop from 0.92 either way.
	if sellStoreExit.hiding then
		sellStoreExit.cancel()
	end
...
local function makePanel(title, titleColor, desktopSize)
	...
	if IS_MOBILE then
		-- harborheist-kqbq.21.1: grab-handle pill that signals draggability.
		-- kqbq.21.1 spec: 36x4, text.tertiary at ~0.5 alpha, corners.pill,
		-- centered at the top edge inside the drag zone. Mobile-only (the
		-- IS_MOBILE branch), decorative (a Frame never intercepts the drag).
		...
		-- harborheist-3mo7.1.4: zone widened 60→80px — the 60px strip was
		-- borderline for comfortable thumb interaction; 80px keeps clear
		-- separation from the content below (list starts at headerY+40+12).
		local DRAG_DISMISS_ZONE = 80
		local dragSurface = Instance.new("TextButton")
		dragSurface.Name = "DragDismiss"
		dragSurface.Size = UDim2.new(1, 0, 0, DRAG_DISMISS_ZONE)
		...
		dragSurface.InputBegan:Connect(function(input)
			if input.UserInputType == Enum.UserInputType.Touch and activePanel == panel then
				...
				dragDy = 0
				-- harborheist-3mo7.1.4: confirm the gesture was recognized —
				-- the pill brightens while the finger is down (restored on
				-- TouchEnded). Restores run even on the dismiss path because
				-- destroy() removes the whole panel, so no stale state leaks.
		...
		UserInputService.TouchEnded:Connect(function(input)
			if input == dragInput then
				...
				grabber.BackgroundTransparency = 0.5
				if dragDy > 90 then
					-- harborheist-3mo7.1.4: haptic confirms the committed
					-- dismiss (bead acceptance). Play BEFORE hidePanels(),
					-- not inside/after it — hidePanels tears down the context
					-- menu and starts the exit animation; side effects must
					-- not depend on ordering with that teardown.
					playHaptic("PressEnd")
					hidePanels()
				else
...
-- [harborheist-kqbq.22.3] Forward declarations — shopPanel/collectionPanel/
-- shopList/collectionList are declared ~:3864-4301 (AFTER hidePanels and
-- showPanel), so the stash/restore hooks can't reference them directly.
-- Assigned once at the shop section below; the stash/restore closures
-- resolve through these upvalues at call time (never nil at runtime — the
-- hooks only fire on shopPanel/collectionPanel, which exist by then).
local shopPanelRef, collectionPanelRef, shopListRef, collectionListRef = nil, nil, nil, nil
local stashedScrollPositions = { shop = nil, collection = nil }
...
hidePanels = function()
	...
	if activePanel then
		...
		if shopPanelRef and panel == shopPanelRef then
			stashedScrollPositions.shop = shopListRef.CanvasPosition.Y
		elseif collectionPanelRef and panel == collectionPanelRef then
			stashedScrollPositions.collection = collectionListRef.CanvasPosition.Y
		end
		if IS_MOBILE then
...
-- [harborheist-kqbq.21.1] One-time first-open hint: on the FIRST panel open
-- of a session, the sheet does a subtle 6px up-down bounce (2 cycles, Sine
-- InOut) after the entrance settles — teaches drag-to-dismiss without a
-- word. Fires once per session; never on desktop (bottom-sheet pattern is
-- mobile-only).
local panelBounceHintShown = false
...
local function showPanel(panel)
	...
	-- harborheist-kqbq.22.3: restore the stashed scroll position ONE FRAME
	-- after content render — AutomaticCanvasSize settles post-layout, and a
	-- pre-layout write is a classic no-op. Clamp to the max canvas offset so
	-- a shrunk list (a2ug.9 collapse, a sell) never restores into blank space.
	local stashedKey = (shopPanelRef and panel == shopPanelRef) and "shop"
		or (collectionPanelRef and panel == collectionPanelRef) and "collection" or nil
	local stashedY = stashedKey and stashedScrollPositions[stashedKey]
	if stashedKey and stashedY and stashedY > 0 then
		local list = (stashedKey == "shop") and shopListRef or collectionListRef
		game:GetService("RunService").RenderStepped:Wait()
		local maxOffset = math.max(0, list.AbsoluteCanvasSize.Y - list.AbsoluteWindowSize.Y)
		list.CanvasPosition = Vector2.new(0, math.clamp(stashedY, 0, maxOffset))
		stashedScrollPositions[stashedKey] = nil
	end
end
...
task.spawn(function()
	...
	while true do
		-- etj2.2.6: no DataStore-health carve-out here — CLAIM stays live in
		-- degraded mode (the persistent banner carries the warning), so the
		-- pulse no longer needs to suppress itself for an unhealthy store.
		if state
...
applyScrollbarAutoHide(inventoryList)
...
applyScrollbarAutoHide(collectionList)
...
renderCollection = function()
	...
	if book.milestones and #book.milestones > 0 then
		...
		-- harborheist-3mo7.3.34: designed empty state instead of a bare
		-- "No milestones available." label. Compact row variant — the full
		-- hero card would dwarf the rows around it inside a subsection.
		renderEmptyRow(collectionList, {
			icon = "\u{1F3C6}",
			title = "No milestones yet",
			subtitle = "Keep catching and discovering — milestones appear as you progress.",
			order = order,
			accent = Theme.color.status.warn,
		})
	end
end
...
applyScrollbarAutoHide(shopList)
-- harborheist-kqbq.22.3: bind the scroll-restore forward declarations now
-- that all four locals exist (hidePanels/showPanel resolve through them).
shopPanelRef = shopPanel
shopListRef = shopList
collectionPanelRef = collectionPanel
collectionListRef = collectionList
...
-- kqbq.15: content signature skip — avoid re-iterating shop rows on
-- every 1 Hz state push when the visible state (ownership tiers,
-- maxed tracks, affordability of next tier) has not changed.
local lastShopSignature = nil
...
function refreshShop()
	...
	-- kqbq.15: content signature — skip the row-update + maxed-collapse
	-- pass when the shop visible state has not changed. Cash ticks every
	-- second but only matters at affordability boundaries, so the signature
	-- captures per-kind: level + maxed + affordability of next tier rather
	-- than the raw cash value.
	local sigParts = {}
	for _, kind in ipairs({ "rod", "bait", "aquarium", "lock", "alarm", "dock" }) do
		local currentLevel
		if kind == "rod" then currentLevel = state.rodLevel or 1
		elseif kind == "bait" then currentLevel = state.baitLevel or 1
		elseif kind == "aquarium" then currentLevel = state.upgradeLevel or 1
		elseif kind == "lock" then currentLevel = state.lockLevel or 0
		elseif kind == "alarm" then currentLevel = state.alarmLevel or 0
		elseif kind == "dock" then currentLevel = state.dockLevel or 1
		end
		local maxLevel = shopMaxLevels[kind] or 0
		local affordKey = "max"
		if (currentLevel or 0) < maxLevel then
			local nextEntry = shopRows[kind .. (currentLevel + 1)]
			local cost = nextEntry and nextEntry.item and nextEntry.item.cost or 0
			affordKey = ((state.cash or 0) >= cost) and "Y" or "N"
		end
		table.insert(sigParts, kind .. ":" .. tostring(currentLevel) .. ":" .. affordKey)
	end
	local shopSig = table.concat(sigParts, "|")
	if shopSig == lastShopSignature then
		return
	end
	lastShopSignature = shopSig
...
applyScrollbarAutoHide(questList)
...
applyScrollbarAutoHide(raidTargetList)
...
-- ============================================================
-- [harborheist-ux45-workflow-clarity-etj2.2.6] Persistent DataStore-degraded
-- banner. Replaces the old one-shot toast + CLAIM-only disable (both lied:
-- the toast claimed "progress is safe", and the disable implied claim was
-- uniquely dangerous — the server honors every economy action in degraded
-- mode and persists each via the same single-profile UpdateAsync). Copy per
-- docs/DATASTORE_DEGRADED_CONTRACT.md §3.2 and DESIGN_SYSTEM.md §15: warn
-- (not error) color because nothing is broken — actions still count.
-- Visibility is driven by render() off state.dataStoreHealthy.
-- ============================================================
local dataStoreBanner = Instance.new("Frame")
dataStoreBanner.Name = "DataStoreBanner"
-- Desktop: top-center below the raid-banner slot (+6..42). Mobile: top-LEFT
-- below the HUD — a centered banner horizontally overlaps the top-right raid
...
dataStoreBanner.ZIndex = 18
dataStoreBanner.Parent = screenGui
corner(dataStoreBanner, Theme.corners.md)
stroke(dataStoreBanner, 0.7, Theme.color.status.warn, 1.5)

local dataStoreBannerIcon = Instance.new("Frame")
dataStoreBannerIcon.Size = UDim2.new(0, 8, 0, 8)
dataStoreBannerIcon.Position = UDim2.new(0, 14, 0, 9)
dataStoreBannerIcon.BackgroundColor3 = Theme.color.status.warn
dataStoreBannerIcon.ZIndex = 19
dataStoreBannerIcon.Parent = dataStoreBanner
corner(dataStoreBannerIcon, Theme.corners.pill)

makeLabel(dataStoreBanner, {
	Size = UDim2.new(1, -36, 0, 16),
	Position = UDim2.new(0, 28, 0, 5),
	Text = "Saving interrupted — retrying",
	Font = Theme.type.fonts.bold,
	TextSize = Theme.type.sizes.xs,
	TextColor3 = Theme.color.status.warn,
	TextXAlignment = Enum.TextXAlignment.Left,
...
makeLabel(dataStoreBanner, {
	...
	Text = "Everything you earn counts and saves automatically when connection recovers. Stay in game to keep recent progress.",
	Font = Theme.type.fonts.body,
	TextSize = Theme.type.sizes.xxs,
	TextColor3 = Theme.color.text.secondary,
	TextXAlignment = Enum.TextXAlignment.Left,
...
-- chip instead (added to the ACTIONS table above). The "?" sits 44px
-- below the raid-banner slot so the two never overlap.
-- ============================================================
local mobileHelpButton = nil
if IS_MOBILE then
	mobileHelpButton = makeButton(screenGui, {
		Size = UDim2.new(0, 44, 0, 44),
		Position = UDim2.new(1, -12, 0, SAFE_TOP + 50),
	...
	mobileHelpButton.BackgroundTransparency = 0.16
	mobileHelpButton.ZIndex = 18
	stroke(mobileHelpButton, 0.58, Theme.color.text.tertiary, 1.5)
	table.insert(safeTopConsumers, function()
		if mobileHelpButton then
			mobileHelpButton.Position = UDim2.new(1, -12, 0, SAFE_TOP + 50)
		end
	end)
end
...
if IS_MOBILE then
	...
	-- (runs for BOTH scrollable and non-scrollable cases)
	...
	-- harborheist-review-aug2026-6yp6.2: handle stored at module level so
	-- it's trackable/disconnectable (was fire-and-forget). Session-lifetime
	-- frame, so never read — annotated to hold the selene baseline.
	-- selene: allow(unused_variable)
	local actionStackScrollConn = stack:GetPropertyChangedSignal("CanvasPosition"):Connect(function()
...
local function render()
	...
	-- TASK 10.5 + harborheist-ux45-workflow-clarity-etj2.2.6: DataStore-degraded
	...
	-- harborheist-ux45-workflow-clarity-etj2.2.6: CLAIM is no longer disabled
	-- in degraded mode — the server honors it and checkpoints like every
	-- other economy action (contract §2 row 1). The persistent banner is the
	-- honest signal; a disabled button was cosmetic and exploiter-bypassable.
	...
	if not ob.HasCaughtFirstFish then
		...
		elseif casting then
			...
			if currentPromptStage == "firstCast" then
				currentPromptStage = nil
				-- harborheist-3mo7.3.15: animated exit (no-ops when hidden)
				onboardingExit.exit()
			end
	...
	-- harborheist-3mo7.3.15: the exit wind-down keeps .Visible true for
	-- 0.16s (deferred hide) — treat a prompt mid-exit as already hidden
	-- here, or a catch landing in that window would be skipped by the
	-- gate and the comparison lost (lastCarriedCount already advanced).
	local sellStoreEffectivelyHidden = not sellStorePrompt.Visible or sellStoreExit.hiding
	if carriedCount > lastCarriedCount
		and not sellStorePromptShown
		and not (state.onboarding or {}).HasSeenSellStoreComparison
		and sellStoreEffectivelyHidden then
...
biteMarkerGlow.Size = UDim2.new(0, 18, 1, 10)
...
biteMarkerGlow.BackgroundTransparency = 0.50
...
helpPanel, helpContent, helpClose = makePanel("HOW TO PLAY", Theme.color.accent.base, UDim2.new(0, 360, 0, 480))
...
local SHORTCUT_ROWS = {
	-- [harborheist-a2ug.13] Gameplay how-to-play tips (both platforms).
	{ tip = true, desc = "Fish in the glowing zone at your dock" },
...
for idx, row in ipairs(SHORTCUT_ROWS) do
	...
	-- keyboard); gameplay tip rows render on both platforms.
	local renderRow = true
	if IS_MOBILE and row.key and not row.tip then
		local mobileKeys = { F = true, G = true, C = true, T = true, S = true, Q = true, R = true, B = true }
		renderRow = mobileKeys[row.key] == true
	end
	...
	if renderRow then
		...
		if row.tip then
			-- Gameplay tip: full-width text, no key chip
			makeLabel(rowFrame, {
				Size = UDim2.new(1, 0, 1, 0),
				Position = UDim2.new(0, 0, 0, 0),
				Text = "\u{2022} " .. row.desc,
				Font = Theme.type.fonts.med,
				TextSize = Theme.type.sizes.sm,
				TextColor3 = Theme.color.accent.soft,
				TextXAlignment = Enum.TextXAlignment.Left,
...
-- [harborheist-a2ug.13] Wire HELP action button (desktop) and mobile "?" button
-- to toggleHelpPanel. Both are gated (help only exists on desktop; the mobile
-- "?" button is IS_MOBILE-only).
if actionButtons.help then
	actionButtons.help.Activated:Connect(toggleHelpPanel)
end
if mobileHelpButton then
	mobileHelpButton.Activated:Connect(toggleHelpPanel)
end
