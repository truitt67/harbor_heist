...
-- Current layout mode (updated on resize)
local currentLayoutMode = getLayoutMode()
...
local function pressFeedback(button, hoverGlow)
	...
	if not IS_MOBILE then
		...
		-- Fresh-eyes fix: MouseLeave does NOT fire when a button is
		-- destroyed mid-hover (list rebuild under the cursor, panel swap) —
		-- without this the pointing-hand cursor sticks for the rest of the
		-- session. Over-clears in the rare case another button is hovered
		-- when this one dies; that self-heals on the next MouseEnter, which
		-- is the better failure side (arrow > phantom hand).
...
local function updatePanelSizing()
	for _, entry in ipairs(registeredPanels) do
		if entry.panel and entry.panel.Parent then
...
-- harborheist-vasr: forward-declared — defined after renderInventory (the
-- menu actions reuse invStoreAllBtn / friendlyFailureReason from this file).
